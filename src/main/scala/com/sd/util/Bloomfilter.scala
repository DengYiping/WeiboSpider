...
/**
  * Bloom filter can be passed into high level function as a filter. By default it will filter those which are not in the set.
  */
...
/**
  *
  * @param filter A set like filter
  * @tparam T
  */
class Deduplicator[T](filter:(T => Boolean)) extends (T => Boolean){
  def apply(a:T) = !filter(a)
}
